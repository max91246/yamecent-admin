...
Route::group([
    ...
    $route->group(['prefix' => 'admin'], function ($route) {
        ...
        $route->get('test/list', 'Admin\TestController@list');

        // 文章管理
        $route->get('article/list', 'Admin\ArticleController@articleList');
        $route->get('article/add', 'Admin\ArticleController@articleAddView');
        $route->post('article/add', 'Admin\ArticleController@articleAdd');
        $route->get('article/update/{id}', 'Admin\ArticleController@articleUpdateView');
        $route->post('article/update/{id}', 'Admin\ArticleController@articleUpdate');
        $route->post('article/del/{id}', 'Admin\ArticleController@articleDel');

        // 會員管理
        $route->get('member/list', 'Admin\MemberController@memberList');
        $route->get('member/add', 'Admin\MemberController@memberAddView');
        $route->post('member/add', 'Admin\MemberController@memberAdd');
        $route->get('member/update/{id}', 'Admin\MemberController@memberUpdateView');
        $route->post('member/update/{id}', 'Admin\MemberController@memberUpdate');
        $route->post('member/del/{id}', 'Admin\MemberController@memberDel');
        // 會員資格管理
        $route->post('member/membership/{id}/activate', 'Admin\MemberController@activateMembership');
        $route->post('member/membership/{id}/revoke',   'Admin\MemberController@revokeMembership');

        // 資產管理
        $route->get('member/balance/list', 'Admin\MemberBalanceController@balanceList');
        $route->get('member/balance/add',  'Admin\MemberBalanceController@balanceAddView');
        $route->post('member/balance/add', 'Admin\MemberBalanceController@balanceStore');

        // 留言管理
        $route->get('comment/list',           'Admin\CommentController@commentList');
...
Route::group([
    ...
    // 會員搜尋 AJAX（不需 RBAC，只需登入）
    $route->get('admin/member/search', 'Admin\MemberController@memberSearch');
    //图片上传
...
// H5 行動介面（Vue 3 SPA entry point）
Route::get('h5/{any?}', function () {
    $file = public_path('h5/index.html');
    return file_exists($file) ? response()->file($file) : abort(404, 'H5 尚未 build，請先執行 npm run build');
})->where('any', '.*')->middleware(['install.check']);
